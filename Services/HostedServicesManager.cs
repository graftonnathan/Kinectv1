    ...
    public class HostedServicesManager
    {
        ...
        public void RegisterService(IHostedService service)
        {
            ...
            lock (_lock)
            {
                ...
                {
                    Console.WriteLine($"?? Service '{service.ServiceName}' already registered, skipping");
                    return;
                }
        ...
        public async Task StopAllAsync(TimeSpan? timeout = null)
        {
            ...
            try
            {
                ...
                foreach (var service in servicesToStop)
                {
                    ...
                    {
                        Console.WriteLine($"?? Overall timeout reached, force stopping remaining services");
                    ...
                    try
                    {
                        ...
                        if (completedTask == stopTask)
                        {
                            ...
                            {
                                Console.WriteLine($"?? Service stop error (graceful): {service.ServiceName}: {ex.Message}");
                            }
                        }
                        else
                        {
                            // Service timed out, force stop
                            Console.WriteLine($"?? Service {service.ServiceName} timed out after 2s, marking as force-stopped");
                            forcedKillCount++;
