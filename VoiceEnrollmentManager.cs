...
public static class VoiceEnrollmentManager
{
    ...
    private static readonly List<float[]> _enrollmentSamples = new();
    ...
    public static void StartEnrollment(string name)
    {
        ...
        Console.WriteLine($"?? Started enhanced voice enrollment for '{name}' - please speak clearly {RequiredSamples} times");
        Console.WriteLine($"   ?? Enhanced enrollment for better accuracy: {RequiredSamples} voice samples required");
        Console.WriteLine($"   ?? Speak when you see high audio levels (green bar), pause between samples");
        Console.WriteLine($"   ?? Minimum {MinSampleInterval.TotalSeconds}s between samples for voice diversity");
    ...
    public static void ProcessVoiceSample(float[] embedding)
    {
        ...
        else
        {
            ...
            Console.WriteLine($"   ?? Please speak again... {remaining} more sample(s) needed for enhanced accuracy");
            
            // Provide milestone feedback
            if (currentCount == 3)
                Console.WriteLine($"   ?? Great! You've completed the basic requirement. {remaining} more for enhanced accuracy.");
            else if (currentCount == 7)
                Console.WriteLine($"   ?? Almost there! Just {remaining} more samples for maximum accuracy.");
        }
    }
    ...
    private static void CompleteEnrollment()
    {
        ...
        try
        {
            ...
            Console.WriteLine($"? Enhanced voice enrollment completed for '{name}' with {_enrollmentSamples.Count} samples");
            Console.WriteLine($"?? Voice recognition accuracy should be significantly improved with {_enrollmentSamples.Count} voice embeddings");
