...
public class PersonMemory
{
    ...
    public List<float[]> FaceEmbeddings { get; set; } = new(); // Renamed for clarity
    public List<float[]> VoiceEmbeddings { get; set; } = new(); // Added voice embeddings
}

public class MemoryIndex
{
    public List<PersonMemory> People { get; set; } = new();
}
...
public static class MemoryStore
{
    ...
    private static MemoryIndex _index = new();
    ...
    public static void Init()
    {
        ...
        if (File.Exists(MemoryFile))
        {
            try
            {
                ...
                _index = JsonSerializer.Deserialize<MemoryIndex>(json) ?? new MemoryIndex();
        ...
        Console.WriteLine("MemoryStore initialized with voice support");
    }
    ...
    private static void MigrateLegacyEmbeddings(PersonMemory person, string json)
    {
        try
        {
            // Parse legacy format and migrate to FaceEmbeddings
            using var doc = JsonDocument.Parse(json);
            var people = doc.RootElement.GetProperty("People");
            
            foreach (var p in people.EnumerateArray())
            {
                if (p.GetProperty("Name").GetString() == person.Name)
                {
                    if (p.TryGetProperty("Embeddings", out var embeddings))
                    {
                        foreach (var emb in embeddings.EnumerateArray())
                        {
                            var embArray = new List<float>();
                            foreach (var val in emb.EnumerateArray())
                            {
                                embArray.Add(val.GetSingle());
                            }
                            person.FaceEmbeddings.Add(embArray.ToArray());
                        }
                    }
                    break;
                }
    ...
    public static void AddVoiceEmbedding(string name, float[] embedding)
    {
        ...
        Save();
        Console.WriteLine($"Added voice embedding for {name}. Total voice embeddings: {person.VoiceEmbeddings.Count}");
    }
    ...
    public static int FlushAllVoiceEmbeddings()
    {
        try
        {
            ...
            Save();
            Console.WriteLine($"🗑️ Flushed {totalRemoved} voice embeddings from all speakers");
            ...
            try
            {
                if (Directory.Exists(VoicesRoot))
                {
                    ...
                    Directory.CreateDirectory(VoicesRoot);
                    Console.WriteLine("🗑️ Cleaned up voice storage folders");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"⚠️ Warning: Could not clean voice folders: {ex.Message}");
            }
    ...
    public static int FlushVoiceEmbeddings(string name)
    {
        try
        {
            ...
            if (person == null)
            {
                Console.WriteLine($"⚠️ No person found with name: {name}");
                return 0;
            }
    ...
    public static (string name, float score)? MatchBestVoice(float[] queryEmb, float thresholdCos = 0.40f)
    {
        ...
        Console.WriteLine($"🔍 Searching through {_index.People.Count} people for voice match...");
        ...
        foreach (var p in _index.People)
        {
            ...
            {
                var cos = Cosine(queryEmb, e);
                Console.WriteLine($"   {p.Name}: similarity = {cos:F3}");
        ...
        Console.WriteLine($"🔍 Checked {totalVoiceEmbeddings} voice embeddings. Best: {best} ({bestCos:F3}), Threshold: {thresholdCos:F2}");
    ...
    private static float Cosine(float[] a, float[] b)
    {
        ...
        {
            Console.WriteLine($"⚠️ Embedding dimension mismatch: {a.Length} vs {b.Length}");
            return 0f;
        }
        ...
        var norm = MathF.Sqrt(na) * MathF.Sqrt(nb) + 1e-9f;
        return dot / norm;
    }
